...
Breweries is a dataset baked into mapview for testing.

```{r}
mapview(breweries)
```

Just looking at the data for breweries.

```{r}
brew <- as.data.frame(breweries)
brew
```
...
Looking at the use_clean column.
